...
/*                                main()                                */
    ...
    /* -------------------------------------------------------------------- */
       // args = ogr.GeneralCmdLineProcessor( args );
        ...
        {
            logger("!!!pszDataSource == null iArg="+iArg);
            logger("!!!pszDataSource == null args[iArg]="+ args[iArg]);
            for(int i=0; i<args.length;i++){
                logger("!!!pszDataSource == null args="+ args[i]);
            }
            ...
            else if( args[iArg].equalsIgnoreCase("-nlt") && iArg < args.length-1 )
            {
                ...
                {
                    logger("-nlt " + args[iArg+1] + ": type not recognised.");
                   // System.exit( 1 );
                }
                iArg++;
            }
            ...
            else if( args[iArg].equalsIgnoreCase("-fieldTypeToString") && iArg < args.length-1 )
            {
                ...
                while(tokenizer.hasMoreElements())
                {
                    ...
                    {
                        Usage();
                        throw new Exception("1###Unhandled type for fieldtypeasstring option : " + token);
            ...
            */
            else if( args[iArg].equalsIgnoreCase("-clipsrc") && iArg < args.length-1 )
            {
                        ...
                        (args[iArg+1].length() >= 12 && args[iArg+1].substring(0, 12).equalsIgnoreCase("MULTIPOLYGON") ) )
                {
                    ...
                    {
                        Usage();
                        throw new Exception("2###FAILURE: Invalid geometry. Must be a valid POLYGON or MULTIPOLYGON WKT\n\n");
            ...
            else if( args[iArg].equalsIgnoreCase("-zfield") && iArg < args.length-1 )
            {
            ...
            else if(args[iArg] != null && args[iArg].length()>0 && args[iArg].charAt(0) == '-') {
                Usage();
                logger("!!!case37");
            }
                ...
                logger("!!!pszDataSource ENTER iArg="+iArg);
                for(int i=0; i<args.length;i++){
                    logger("!!!pszDataSource ENTERargs="+ args[iArg]);
                }
            }
            else {
                logger("!!!case40");
        ...
        {
            Usage();
            throw new Exception("3###FAILURE: cannot use -preserve_fid and -explodecollections at the same time\n\n" );
        ...
        if( bClipSrc && pszClipSrcDS != null)
        {
            ...
            {
                Usage();
                throw new Exception("4###FAILURE: cannot load source clip geometry\n\n" );
        ...
        else if( bClipSrc && poClipSrc == null )
        {
            ...
            {
                Usage();
                throw new Exception("5###FAILURE: -clipsrc must be used with -spat option or a\n" +
        ...
        if( pszClipDstDS != null)
        {
            ...
            {
                Usage();
                throw new Exception("6###FAILURE: cannot load dest clip geometry\n\n" );
    ...
    /* -------------------------------------------------------------------- */
        if( poDS == null )
        {
            ...
            throw new Exception("7###FAILURE:Unable to open datasource ` " + pszDataSource + "' with the following drivers.");
    ...
    /* -------------------------------------------------------------------- */
        ...
        logger(" ***pszDestDataSource "+pszDestDataSource);
        if( bUpdate )
        {   logger("bUpdate 987 pszDestDataSource "+pszDestDataSource);
            poODS = ogr.Open( pszDestDataSource, true );
            if( poODS == null )
            {   logger("bUpdate 987 first Open failed ");
                if (bOverwrite || bAppend)
                {
                    poODS = ogr.Open( pszDestDataSource, false );
                    if ( poODS == null )
                    {   logger("bUpdate 987 second Open failed ");
            ...
            {
                throw new Exception("9###WARNING: Datasource creation options ignored since an existing datasource\n" +
                        "         being updated." );
            }
    ...
    /* -------------------------------------------------------------------- */
        ...
        if( !bUpdate )
        {
            ...
            if( poDriver == null )
            {
                String error= "10###Unable to find driver `" + pszFormat +"The following drivers are available:" ;
            ...
            {
                throw new Exception( "11###"+pszFormat + " driver does not support data source creation.");
               // System.exit( 1 );
            }
    ...
    /* -------------------------------------------------------------------- */
                    ...
                    (pszDestDataSource.endsWith(".shp") || pszDestDataSource.endsWith(".SHP")))
            {
                ...
                if (!f.exists())
                {
                    ...
                    {
                        throw new Exception(
                                "12###Failed to create directory " + pszDestDataSource + "\n" +
    ...
    /* -------------------------------------------------------------------- */
            logger("!bUpdate="+pszDestDataSource);
            logger("!papszDSCO="+papszDSCO);
            poODS = poDriver.CreateDataSource( pszDestDataSource, papszDSCO );
            if( poODS == null )
            {
                throw new Exception( "13###"+ pszFormat + " driver failed to create "+ pszDestDataSource );
               // System.exit( 1 );
            }
        }
    ...
    /* -------------------------------------------------------------------- */
        if( pszOutputSRSDef != null )
        {
            ...
            {
                throw new Exception( "14###Failed to process SRS definition: " + pszOutputSRSDef );
               // System.exit( 1 );
            }
        }
    ...
    /* -------------------------------------------------------------------- */
        if( pszSourceSRSDef != null )
        {
            ...
            {
                throw new Exception( "14###Failed to process SRS definition: " + pszSourceSRSDef );
               // System.exit( 1 );
            }
        }
    ...
    /* -------------------------------------------------------------------- */
        if( pszSQLStatement != null )
        {
            ...
            if( pszWHERE != null )
                throw new Exception( "15###-where clause ignored in combination with -sql." );
            if( papszLayers.size() > 0 )
                throw new Exception( "16###layer names ignored in combination with -sql." );
            ...
            if( poResultSet != null )
            {
                ...
                if (bDisplayProgress)
                {
                    if (!poResultSet.TestCapability(ogr.OLCFastFeatureCount))
                    {
                        ...
                        throw new Exception("17###Progress turned off as fast feature count is not available.");
...
/* -------------------------------------------------------------------- */
                ...
                {
                    throw new Exception(
                            "18###Terminating translation prematurely after failed" +
    ...
    /* -------------------------------------------------------------------- */
            if ( papszLayers.size() == 0)
            {
                     ...
                     iLayer++ )
                {
                    ...
                    if( poLayer == null )
                    {
                        throw new Exception("19###FAILURE: Couldn't fetch advertised layer " + iLayer + "!");
                       // System.exit( 1 );
                    }
    ...
    /* -------------------------------------------------------------------- */
            else
            {
                ...
                logger("@@@Layers GET papszLayers="+papszLayers);
                logger("@@@Layers GET papszLayers.size="+papszLayers.size());
                for( int iLayer = 0;
                     iLayer < papszLayers.size();
                     iLayer++ )
                {
                    logger("@@@Layers GET iLayer="+iLayer);
                    Layer poLayer=null;
                    //Layer        poLayer = poDS.GetLayerByName((String)papszLayers.get(iLayer));

                    int n = papszLayers.size();
                    StringBuffer resultpapszLayers = new StringBuffer();
                    for (int i = 0; i < n; i++) {
                        resultpapszLayers.append(papszLayers.elementAt(i).toString());
                    }

                    String poLayerString = resultpapszLayers.toString();

                    logger("@@@Layers GET poLayerString="+poLayerString);

                    try {
                        poLayer = poDS.GetLayerByName(poLayerString);
                        logger("@@@Layers GET poLayer=" + poLayer);
                    }catch(NullPointerException ex){
                        logger("@@@Layers NULL layer="+iLayer);
                    }
                    if( poLayer == null )
                    {
                        throw new Exception("19###FAILURE: Couldn't fetch advertised layer " + (String)papszLayers.get(iLayer) + "!");
                       // System.exit( 1 );
                    }
...
/* -------------------------------------------------------------------- */
                 ...
                 iLayer++ )
            {
                ...
                if( pszWHERE != null )
                {
                    ...
                    {
                        throw new Exception("20###FAILURE: SetAttributeFilter(" + pszWHERE + ") failed.");
                        //if (!bSkipFailures)
                          //  System.exit( 1 );
                    }
                }
                ...
                if (bDisplayProgress)
                {
                    ...
                    {
                        bDisplayProgress = false;
                        throw new Exception("21###Progress turned off as fast feature count is not available.");
                 ...
                 iLayer++ )
            {
                        ...
                        && !bSkipFailures )
                {
                    try {
                        throw new Exception(
                                "22###Terminating translation prematurely after failed\n" +
                                        "translation of layer " + poLayer.GetLayerDefn().GetName() + " (use -skipfailures to skip errors)");
                    }catch(NullPointerException e){
                        logger("23###Terminating translation prematurely after failed\n" +e.getMessage());
                        e.printStackTrace();
                    }
    ...
    static Geometry LoadGeometry( String pszDS,
                                  String pszSQL,
                                  String pszLyr,
                                  String pszWhere) throws Exception {
        DataSource       poDS;
        Layer            poLyr;
        Feature          poFeat;
        ...
        {
            poDS.delete();
            throw new Exception("24###Failed to identify source layer from datasource.");
           // return null;
        ...
        while ((poFeat = poLyr.GetNextFeature()) != null)
        {
            ...
            if (poSrcGeom != null)
            {
                ...
                else
                {
                    ...
                    poDS.delete();
                    throw new Exception("25###ERROR: Geometry not of polygon type.\n" );
                }
            }
        }
    ...
    static boolean TranslateLayer( DataSource poSrcDS,
                                   ...
                                   String pszZField,
                                   String pszWHERE,
                                   ProgressCallback pfnProgress) throws Exception

    {
        ...
        try {
            ...
            logger("pszNewLayerName is null");
        }
    ...
    /* -------------------------------------------------------------------- */
        ...
        if( bTransform )
        {
            ...
            if( poSourceSRS == null )
            {
                throw new Exception("26###Can't transform coordinates, source layer has no\n" +
            ...
            {
                String pszWKT = null;
                String error="Failed to create coordinate transformation between the\n" +
    ...
    /* -------------------------------------------------------------------- */
        ...
        poDstLayer = null;
        try {
            //logger("@@@ERROR1 poDstDS="+poDstDS);
           // logger("@@@ERROR1 pszNewLayerName="+pszNewLayerName);
            poDstLayer = poDstDS.
                    GetLayerByName(
                            pszNewLayerName);
    ...
    /* -------------------------------------------------------------------- */
        if( poDstLayer != null && bOverwrite )
        {
            ...
            {
                throw new Exception(
                        "27###DeleteLayer() failed when overwrite requested." );
                //return false;
            }
            poDstLayer = null;
        }
        ...
        if( poDstLayer == null )
        {
            ...
            try {
                if (poDstDS.
                    ...
                    throw new Exception(
                            "28###Layer " + pszNewLayerName + "not found, and CreateLayer not supported by driver.");
                    //return false;
                }
            }catch(NullPointerException e){
                logger(e.getMessage());
    ...
    /* -------------------------------------------------------------------- */
        ...
        {
            throw new Exception("28###FAILED: Layer " + pszNewLayerName + "already exists, and -append not specified.\n" +
        ...
        else
        {
            if( papszLCO.size() > 0 )
            {
                throw new Exception("29###WARNING: Layer creation options ignored since an existing layer is\n" +
                        "         being appended to." );
            }
        }
    ...
    /* -------------------------------------------------------------------- */
            ...
            for( iField=0; iField < papszSelFields.size(); iField++)
            {
                ...
                if (iSrcField >= 0)
                {
                    ...
                    else if (poDstLayer.CreateField( oFieldDefn ) == 0)
                    {
                        ...
                        {
                            throw new Exception(
                                    "30###The output driver has claimed to have added the " + oFieldDefn.GetNameRef() + " field, but it did not!");
                        }
                        else
                        {
                ...
                else
                {
                     if( !bSkipFailures )
                        return false;
                    throw new Exception("31###Field '" + (String)papszSelFields.get(iField) + "' not found in source layer.");
        ...
        else
        {
            ...
            if (poDstFDefn == null)
            {
                throw new Exception("poDstFDefn == NULL.\n" );
               // return false;
            }
            ...
            for(int iPart = 0; iPart < nIters; iPart++)
            {
                ...
                if( poDstFeature.SetFromWithMap( poFeature, 1, panMap ) != 0 )
                {
                 ...
                 //   return false;
                    ...
                    throw new Exception(
                            "32###Unable to translate feature " + poFeature.GetFID() + " from layer " +
                                    poSrcFDefn.GetName() );
                ...
                /*if (poDstFeature.GetGeometryRef() != null && dfGeomOpParam > 0)
                    ...
                    if( poCT != null )
                    {
                        ...
                        if( eErr != 0 )
                        {
                            ...
                            throw new Exception("33###Failed to reproject feature" + poFeature.GetFID() + " (geometry probably out of source or destination SRS).");
